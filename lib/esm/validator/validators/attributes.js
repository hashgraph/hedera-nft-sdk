...
const attributesValidator = instance => {
  ...
  attributes.map(attribute => {
    ...
    if ((attribute.display_type === 'datetime' || attribute.display_type === 'date') && !Number.isInteger(attribute.value)) {
      errors.push({
        type: 'attribute',
        msg: `Trait ${attribute.trait_type} of type '${attribute.display_type}' requires integer value, got type: ${typeof attribute.value}`
      });
    }
  });
  return errors;
