...
export const createRoyaltyFeeFunction = ({
  ...
  validatePropsForRoyaltyFeeFunction({
    collectorAccountId,
    numerator,
    denominator
  });
  const royaltyFee = new CustomRoyaltyFee().setFeeCollectorAccountId(collectorAccountId).setNumerator(numerator).setDenominator(denominator);
  if (allCollectorsAreExempt) {
    royaltyFee.setAllCollectorsAreExempt(allCollectorsAreExempt);
  }
  if (fallbackFee) {
    royaltyFee.setFallbackFee(createFixedFeeFunction({
