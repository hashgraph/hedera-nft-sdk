...
export async function mintToken(metaData, tokenId, supplyKey, client) {
  ...
  const transaction = new TokenMintTransaction().setTokenId(tokenId).setMaxTransactionFee(new Hbar(mintingMaxTransactionFee)).setMetadata(CIDs).freezeWith(client);
