...
export const dictionary = {
  ...
  validation: {
    ...
    csvFileIsEmpty: path => path ? `No metadata found in CSV file "${getFullSystemPath(path)}".` : 'No metadata found in CSV file.',
    errorInRow: (line, error) => `Error at: line number ${typeof line === 'number' ? line + 1 : line} in ${getFullSystemPath('exampleCSV.csv')}\n${error}`,
    missingAttributesInRow: (csvFilePath, row) => ` - "${getFullSystemPath(csvFilePath)}" in row ${row}`,
    ...
    arrayOfObjectsValidationError: (fileName, error) => `Error at: ${getFullSystemPath(fileName)} - ${error}`,
