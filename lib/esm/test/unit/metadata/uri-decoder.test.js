...
describe('uriDecoder', () => {
  it('decodes IPFS metadata containing "ipfs://" prefix using provided ipfsGateway', () => {
    const nfts = [generateNFTDetails(IPFS_PREFIXED_HASH, 1)];
    const expected = [{
  ...
  it('decodes IPFS metadata that does not contain "ipfs://" prefix using provided ipfsGateway', () => {
    const nfts = [generateNFTDetails(HASH_WITHOUT_IPFS_PREFIX, 2)];
    const expected = [{
  ...
  it('does not alter https:// prefixed metadata', () => {
    const nfts = [generateNFTDetails(HTTPS_URL, 3)];
    const expected = [{
  ...
  it('throws an error if ipfs metadata is provided without an ipfsGateway', () => {
    const nfts = [generateNFTDetails(IPFS_PREFIXED_HASH, 4)];
    expect(() => uriDecoder(nfts)).toThrow('IPFS gateway is required');
  });
  it('decodes various types of metadata correctly', () => {
    const nfts = [generateNFTDetails(IPFS_PREFIXED_HASH, 1), generateNFTDetails(HASH_WITHOUT_IPFS_PREFIX, 2), generateNFTDetails(HTTPS_URL, 3)];
    const expected = [{
      metadata: IPFS_PREFIXED_HASH.replace('ipfs://', EXAMPLE_IPFS_GATEWAY),
      serialNumber: 1
    }, {
      metadata: EXAMPLE_IPFS_GATEWAY + HASH_WITHOUT_IPFS_PREFIX,
      serialNumber: 2
    }, {
      metadata: HTTPS_URL,
      serialNumber: 3
