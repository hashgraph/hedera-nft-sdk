...
describe('calculateRarityFromOnChainData', () => {
  it('should return an array of RarityResult objects', async () => {
    ...
    const limit = 100;
    getNftMetadataFromCollection.mockResolvedValue(nftFromNode);
  ...
  it('should thrown error if metadata not contain attributes', async () => {
    ...
    const ipfsGateway = 'https://ipfs.io/ipfs/';
    const limit = 100;
    getNftMetadataFromCollection.mockResolvedValue([{
      isSuccessful: true,
      metadata: {
        creator: 'Hedera4'
      }
    }]);
    await expect(calculateRarityFromOnChainData(network, tokenId, ipfsGateway, limit)).rejects.toThrowError(dictionary.errors.rarity.attributeNotFoundInObject(JSON.stringify({
      creator: 'Hedera4'
    })));
  });
});
