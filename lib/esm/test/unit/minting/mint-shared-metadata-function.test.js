...
describe('mintSharedMetadataFunction', () => {
  it('should return correct metadata', async () => {
    ...
    expect(result).toEqual(expect.arrayContaining([{
      content: MOCK_META_DATA,
      serialNumber: 1
    }]));
    expect(mintToken.mock.calls.length).toBe(Math.ceil(10 / 2));
  });
  it('should handle amount less than batchSize correctly', async () => {
    ...
    expect(mintToken.mock.calls.length).toBe(Math.ceil(1 / 2));
  });
  it('should handle batchSize of 1 correctly', async () => {
    ...
    expect(mintToken.mock.calls.length).toBe(Math.ceil(1 / 1));
  });
  it('should handle batchSize of 10 correctly', async () => {
    ...
    expect(mintToken.mock.calls.length).toBe(Math.ceil(10 / 10));
  });
  it('should handle error when batchSize is lower than 1', async () => {
    ...
    await expect(mintSharedMetadataFunction({
      client: MOCK_CLIENT,
      metaData: MOCK_META_DATA,
      tokenId: MOCK_TOKEN_ID,
      supplyKey: MOCK_SUPPLY_KEY,
