...
import { JSON_METADATA_INTEGRATION_TESTS_OUTPUT_FOLDER_PATH, CSV_EXAMPLE_WITH_ALL_FIELDS, CSV_EXAMPLE_ONLY_REQUIRED_FIELDS, CSV_EXAMPLE_WITH_MISSING_REQUIRED_FIELDS, CSV_EXAMPLE_ONLY_REQUIRED_FIELDS_AND_HEADERS, LONG_E2E_TIMEOUT } from '../__mocks__/consts';
...
describe('convertMetadataObjectsToJsonFiles Integration Test', () => {
  beforeEach(() => {
    if (!fs.existsSync(JSON_METADATA_INTEGRATION_TESTS_OUTPUT_FOLDER_PATH)) {
      fs.mkdirSync(JSON_METADATA_INTEGRATION_TESTS_OUTPUT_FOLDER_PATH, {
        recursive: true
      });
    }
  });
  afterEach(() => {
    fs.rmSync(JSON_METADATA_INTEGRATION_TESTS_OUTPUT_FOLDER_PATH, {
      recursive: true,
      force: true
    });
  });
