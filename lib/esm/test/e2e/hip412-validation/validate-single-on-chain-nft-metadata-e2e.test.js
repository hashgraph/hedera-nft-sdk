...
import { nftSDK, operatorPrivateKey, NETWORK, IPFS_GATEWAY, LINK_TO_JSON_OBJECT_WITHOUT_ERRORS, AMOUNT_OF_NFTS_TO_MINT_SINGLE, BATCH_SIZE_SINGLE, METADATA_TO_VALIDATE_OBJECT_SERIAL } from '../e2e-consts';
...
describe('E2E test for validating single NFT Metadata Object Against HIP412 schema', () => {
  ...
  it('should successfully validate single metadata object with particular serial number from previously created NFT collection against the HIP412 schema', async () => {
    const validationResponse = await TokenMetadataValidator.validateSingleOnChainNFTMetadata(NETWORK, tokenId, METADATA_TO_VALIDATE_OBJECT_SERIAL, IPFS_GATEWAY);
