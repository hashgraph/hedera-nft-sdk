...
describe('increaseNFTSupply function e2e', () => {
  ...
  testCases.forEach(({
    ...
    it(`Increasing a token supply by ${amount}`, async () => {
      ...
      const nftInfo = await new TokenNftInfoQuery().setNftId(new NftId(TokenId.fromString(tokenId), baseNFT[0].serialNumber)).execute(nftSDK.client);
      ...
      for (const mintedNft of increaseSupplyResult) {
        const nftInfos = await new TokenNftInfoQuery().setNftId(new NftId(TokenId.fromString(tokenId), mintedNft.serialNumber)).execute(nftSDK.client);
