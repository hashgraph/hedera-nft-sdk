...
describe('mintSharedMetadata function e2e', () => {
  ...
  testCases.forEach(({
    ...
    it(`Creating a token and minting ${amount} NFTs into it. Then checking estimated cost`, async () => {
      ...
      const transaction = new TokenMintTransaction().setTokenId(tokenId).setMaxTransactionFee(new Hbar(mintingMaxTransactionFee)).setMetadata(CIDs).freezeWith(nftSDK.client);
