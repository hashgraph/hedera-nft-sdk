...
var _getFullSystemPath = require("../../helpers/get-full-system-path");
/*-
 *
 * Hedera NFT SDK
 *
...
const dictionary = exports.dictionary = {
  errors: {
    ...
    awsUploadingError: message => `Failed to upload file to AWS S3: ${message}`,
    ...
    rarity: {
      attributeTypeNotFound: trait_type => `Attribute ${trait_type} not found in attributes map`,
      attributeNotFoundInFile: fileName => `Attributes not found in file ${fileName}. Please ensure that your metadata file is valid.`,
      attributeNotFoundInObject: object => `Attributes not found in object ${object}. Please ensure that your metadata file is valid.`
    },
    ...
    tooManyRequests: (statusText, status) => `${statusText}. Status code: ${status}`,
    unknownErrorWhileFetching: serialNumber => `Error fetching metadata for serialNumber ${serialNumber}`,
    localization: {
    ...
    metadataBuilder: {
      fieldAlreadySet: field => `${field} can only be set once`,
  ...
  createCollection: {
    ...
    hbarAmountOrAmountAndDenominatingToken: 'Either hbarAmount should be set and both amount and denominatingTokenId should not be set, or amount and denominatingTokenId should be set and hbarAmount should not be set.'
  },
  validation: {
    errorInCellWithHeader: (line, column) => `Error in line number ${line}, column number ${column}. Check if your CSV file is well prepared.`,
    invalidKeysDetected: keys => `Redundant key(s) detected: ['${keys.join("', '")}']`,
    csvFileIsEmpty: path => path ? `No metadata found in CSV file "${(0, _getFullSystemPath.getFullSystemPath)(path)}".` : 'No metadata found in CSV file.',
    errorInRow: (line, error) => `Error at: line number ${typeof line === 'number' ? line + 1 : line} in ${(0, _getFullSystemPath.getFullSystemPath)('exampleCSV.csv')}\n${error}`,
    missingAttributesInRow: (csvFilePath, row) => ` - "${(0, _getFullSystemPath.getFullSystemPath)(csvFilePath)}" in row ${row}`,
    imageForNftNotFound: 'Image for NFT not found. The name of the image file should match its corresponding metadata file name (ex: 1.jpg with 1.json) or specify directly the "image" property.',
    mediaFileNotSupported: 'Media file is not supported.',
    arrayOfObjectsValidationError: (fileName, error) => `Error at: ${(0, _getFullSystemPath.getFullSystemPath)(fileName)} - ${error}`,
  ...
  hederaActions: {
    ...
    pathToMetadataURIsFileNotSupportedInBrowser: 'The "pathToMetadataURIsFile" property is not supported in browser while trying to mint unique metadata. It is required to pass an array of the metadata.'
  },
  mintToken: {
