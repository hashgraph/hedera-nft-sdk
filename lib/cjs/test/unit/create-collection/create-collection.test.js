...
var _createCollection = require("../../../nftSDKFunctions/create-collection");
...
describe('createCollectionFunction', () => {
  it('should create a collection and return a tokenId', async () => {
    const client = _sdk.Client.forTestnet();
    const collectionName = 'test';
    const collectionSymbol = 'test2';
    const tokenId = await (0, _createCollection.createCollectionFunction)({
      client,
  ...
  it('should throw an error when only treasuryAccount is passed', async () => {
    ...
    const collectionName = 'test';
    const collectionSymbol = 'test2';
    await expect((0, _createCollection.createCollectionFunction)({
      client,
      myPrivateKey: _consts.myPrivateKey,
      collectionName,
      collectionSymbol,
      treasuryAccount: '0.0.4321'
    })).rejects.toThrow(_dictionary.dictionary.createCollection.treasuryAccountPrivateKeySignRequired);
  });
  it('should throw an error when only treasuryAccountPrivateKey is passed', async () => {
    ...
    await expect((0, _createCollection.createCollectionFunction)({
      ...
      collectionName,
      collectionSymbol,
      treasuryAccountPrivateKey: _consts.treasuryAccountPrivateKey
    })).rejects.toThrow(_dictionary.dictionary.createCollection.treasuryAccountPrivateKeySignRequired);
  });
  ...
  it('should throw an error if collectionName is not provided', async () => {
    ...
    const collectionSymbol = 'test2';
    const keys = {
      admin: _consts.myPrivateKey,
      supply: _consts.myPrivateKey
    };
    ...
    await expect((0, _createCollection.createCollectionFunction)({
      ...
      treasuryAccount,
      treasuryAccountPrivateKey: _consts.treasuryAccountPrivateKey
    })).rejects.toThrow(_dictionary.dictionary.createCollection.collectionNameRequired);
  });
  it('should throw an error if collectionSymbol is not provided', async () => {
    ...
    await expect((0, _createCollection.createCollectionFunction)({
      ...
      treasuryAccount,
      treasuryAccountPrivateKey: _consts.treasuryAccountPrivateKey
    })).rejects.toThrow(_dictionary.dictionary.createCollection.collectionSymbolRequired);
  });
  it('should create a collection with keys', async () => {
    ...
    const collectionSymbol = 'test2';
    const keys = {
      admin: _sdk.PrivateKey.generateED25519(),
      supply: _consts.myPrivateKey
    };
    const tokenId = await (0, _createCollection.createCollectionFunction)({
      client,
