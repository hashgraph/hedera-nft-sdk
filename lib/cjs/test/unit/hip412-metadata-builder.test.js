...
const MOCKED_ATTRIBUTES_ALL_PARAMS = {
  trait_type: 'color',
  value: 'red',
  display_type: 'percentage',
  max_value: 100
};
const MOCKED_ATTRIBUTES_REQUIRED_PARAMS = {
...
const MOCKED_PROPERTY_1 = {
  key: 'example_key_1',
  value: 'example_value_1'
};
const MOCKED_PROPERTY_2 = {
  key: 'example_key_2',
  value: 'example_value_2'
};
const MOCKED_PROPERTY_3 = {
  key: 'example_key_3',
...
describe('Hip412MetadataBuilder', () => {
  it('should create metadata with only required fields: name, image, and type', () => {
    const builder = new _hip412MetadataBuilder.Hip412MetadataBuilder().setName(MOCKED_NAME).setImage(MOCKED_IMAGE).setType(MOCKED_TYPE);
  ...
  it('should return one validation error when there is no name field provided for metadata object', () => {
    const builder = new _hip412MetadataBuilder.Hip412MetadataBuilder().setImage(MOCKED_IMAGE).setType(MOCKED_TYPE);
    const result = builder.build();
  ...
  it('should return three validation errors when none of the required fields are provided', () => {
    const builder = new _hip412MetadataBuilder.Hip412MetadataBuilder();
    const result = builder.build();
    expect(result.metadata.name).toEqual('');
  ...
  it('should return a full metadata object with all fields filled', () => {
    const builder = new _hip412MetadataBuilder.Hip412MetadataBuilder().setName(MOCKED_NAME).setImage(MOCKED_IMAGE).setType(MOCKED_TYPE).addAttribute(MOCKED_ATTRIBUTES_ALL_PARAMS).addAttribute(MOCKED_ATTRIBUTES_REQUIRED_PARAMS).setCreator(MOCKED_CREATOR).setCreatorDID(MOCKED_CREATOR_DID).setDescription(MOCKED_DESCRIPTION).setChecksum(MOCKED_CHECKSUM).addFile(MOCKED_FILE_ALL_PARAMS).addFile(MOCKED_FILE_REQUIRED_PARAMS).addProperty(MOCKED_PROPERTY_1).addProperty(MOCKED_PROPERTY_2).addProperty(MOCKED_PROPERTY_3).setLocalization(MOCKED_LOCALIZATION);
    const result = builder.build();
    expect(result.metadata.name).toEqual(MOCKED_NAME);
    expect(result.metadata.image).toEqual(MOCKED_IMAGE);
    expect(result.metadata.type).toEqual(MOCKED_TYPE);
...
//# sourceMappingURL=hip412-metadata-builder.test.js.map