...
describe('mintToken', () => {
  it('should return Success status', async () => {
    ...
    const result = await (0, _mintToken.mintToken)(mockMetaData, mockTokenId, _consts.myPrivateKey, mockClient);
    expect(result).toEqual({
      status: _sdk.Status.Success
    });
  });
  ...
  it('should throw error when metadata is 101 characters long', async () => {
    ...
    const mockTokenId = 'tokenId';
    await expect((0, _mintToken.mintToken)(mockMetaData, mockTokenId, _consts.myPrivateKey, mockClient)).rejects.toThrow(_dictionary.dictionary.mintToken.tooLongCID);
  });
});
