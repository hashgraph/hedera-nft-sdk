...
const validate = metadataObject => {
  return _tokenMetadataValidator.TokenMetadataValidator.validateSingleMetadataObject(metadataObject);
};
describe('TokenMetadataValidator.validateSingleObject', () => {
  ...
  it('should return an error for an object with an invalid image MIME type', () => {
    ...
    expect(validationResult.isValid).toBe(false);
    expect(validationResult.errors).toEqual([_dictionary.dictionary.validation.requiredTypeFieldIsMissing]);
  });
  it('should return an error for an object with an invalid attributes structure', () => {
    ...
    expect(validationResult.isValid).toBe(false);
    expect(validationResult.errors).toEqual([_dictionary.dictionary.validation.requiredAttributeFieldMissing]);
  });
});
