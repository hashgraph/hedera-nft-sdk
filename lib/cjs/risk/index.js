...
exports.defaultWeights = exports.defaultRiskLevels = exports.calculateRiskScoreFromTokenId = exports.calculateRiskScoreFromData = exports.calculateRiskLevel = void 0;
...
const defaultWeights = exports.defaultWeights = {
  keys: {
    admin_key: 200,
    wipe_key: 200,
    freeze_key: 50,
...
const defaultRiskLevels = exports.defaultRiskLevels = {
  NORISK: 0,
  LOW: 40,
  MEDIUM: 199,
  HIGH: 200
};
const calculateRiskScoreFromData = _ref => {
  let {
    metadata,
    customWeights,
    customRiskLevels
  ...
  const {
    riskScore,
    riskScoreFactors
  } = calculateRiskScore(metadata, customWeights);
...
exports.calculateRiskScoreFromData = calculateRiskScoreFromData;
const calculateRiskScoreFromTokenId = async _ref2 => {
  let {
    tokenId,
    network = 'mainnet',
    localNodeURL,
  ...
  switch (network) {
    case 'mainnet':
      uri = `${(0, _getMirrorNodeUrlForNetwork.getMirrorNodeUrlForNetwork)('mainnet')}/tokens/${tokenId}/`;
      break;
    case 'testnet':
      uri = `${(0, _getMirrorNodeUrlForNetwork.getMirrorNodeUrlForNetwork)('testnet')}/tokens/${tokenId}/`;
      break;
    case 'previewnet':
      uri = `${(0, _getMirrorNodeUrlForNetwork.getMirrorNodeUrlForNetwork)('previewnet')}/tokens/${tokenId}/`;
      break;
    case 'localNode':
      if (!localNodeURL) {
...
exports.calculateRiskScoreFromTokenId = calculateRiskScoreFromTokenId;
const calculateRiskScore = (metadata, customWeights) => {
  ...
  for (const key in metadata) {
    ...
    if (metadata[typedKey] && typedKey in weights.keys) {
      // If it is, add the associated risk weight to the risk score
      riskScore += weights.keys[typedKey];
      riskScoreFactors[typedKey] = weights.keys[typedKey];
    }
  }
...
const calculateRiskLevel = _ref3 => {
  let {
    score,
    customRiskLevels
  } = _ref3;
